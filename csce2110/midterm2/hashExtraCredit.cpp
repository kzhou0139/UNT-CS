...
// Linear probing, mid-square base 10, double hashing

// // HashTable object .cpp file (Extra credit attempt)
...
// hash function 1
int HashTable::midsquareHash(int val) {
...
// hash function 2
int HashTable::secondHash (int val) {
...
void HashTable::insert(int value) {
  ...
  int hIndex = midsquareHash(value);
  ...
  if (hTable[hIndex].empty()) { // if the location is empty, insert the element at the location
    ...
    int i = 0;
    while (!hTable[hIndex].empty()) { // if the location is not empty, find the next open location using linear probing
      if (i == tableSize) { // collision strategy halt, no where to insert data
        cout << "Insert rejected" << endl;
        return;
      }
      hIndex = (hIndex + i * secondHash(value)) % tableSize; // double hashing
      i++;
    }
  }
...
void HashTable::search(int value) {
  ...
  while (!hTable[hIndex].empty() && hTable[hIndex][0].key != hIndex && i <= tableSize) {
    hIndex = (hIndex + i * secondHash(value)) % tableSize;
    i++;
  }
